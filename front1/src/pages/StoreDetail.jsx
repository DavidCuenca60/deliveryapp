      ...
      <div className="px-6 py-6">
        ...
        <div className="flex flex-col gap-4">
          {products.map((product) => (
            <div key={product.id} className="bg-white  shadow p-4 flex justify-between items-center">
              <div>
                <h3 className="font-bold text-gray-800">{product.name}</h3>
                <p className="text-gray-500 text-sm">{product.description}</p>
                <p className="text-blue-500 font-semibold mt-1">${product.price.toLocaleString()}</p>
              </div>
              <button
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-6 left-0 right-0 px-6">
          <button
            ...
            className="w-full bg-blue-500 text-white py-4 rounded-2xl font-bold shadow-lg  transition flex justify-between px-6"
          >
